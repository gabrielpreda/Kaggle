---
title: European Elections Romania 2019
output:
  html_document:
...
We present here the partial results of the European Elecions 2019 in Romanoa (May 26). 
...
PATH = "../input/ro-european-elections-2019/"
...
presence = smartbind(presence_2607, presence_2608, presence_2609, presence_2610, presence_2611, presence_2612,presence_2613,presence_2614,presence_2615,presence_2616,presence_2617,presence_2618)
...
```{r rbubbly_show1-1,warnings=FALSE}

presence %>% group_by(Time, County, Name, Environment) %>% 
  summarise(Registered.Votes = sum(Votes.Registered),
  Total.Votes = sum(Votes.Total),
  Total.Electors = sum(Electors)) %>% ungroup() -> presence_grouped_total


rbubbly(presence_grouped_total,
        x_column = presence_grouped_total$Registered.Votes, y_column = presence_grouped_total$Total.Votes,
        bubble_column = presence_grouped_total$Total.Electors, color_column = presence_grouped_total$Environment,
        slider_column = presence_grouped_total$Time, text_column = presence_grouped_total$Name,
        x_title="Number registered votes", y_title="Number of total votes",                    
        title="European elections Romnaia 2019\nNumber of votes per county, grouped by Rural/Urban environment",
        x_is_log=FALSE, y_is_log=FALSE,scale_bubble=100)
```
...
presence %>% filter(Date.Hour == max(Date.Hour)) %>% 
  ...
  summarise(Registered.Percent = 100* sum(Votes.Registered)/sum(Electors),
    ...
    Mobile.Percent = 100 * sum(Votes.Mobile) / sum(Electors),
    Total.Percent = 100 * sum(Votes.Total)/sum(Electors),
    Total.Votes = sum(Votes.Total)) %>% ungroup() -> presence_city

presence_city$City <- sapply(presence_city$City, tolower)
roshp$name <- sapply(roshp$name, tolower)
presence_c = presence_county[matchingNames(roshp$name,as.character(presence_city$City)),]
```
...
```{r plot_uat_map}
plot_uat_map <- function(values, main, title, colormap="Blues") {
...
```{r plot_uat_map2}
plot_uat_map(presence_c$Total.Votes, "Total votes", "Total votes per uap")
