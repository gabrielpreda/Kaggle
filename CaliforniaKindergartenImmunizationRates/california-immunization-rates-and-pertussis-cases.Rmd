...
The vaccination allows people to develop immunity to specific diseases. 
The immunization works in parallel in two ways: through vaccination, one individual is protected; and in the same time, 
if a significant part of the population is vaccinated, through [herd immunity](https://en.wikipedia.org/wiki/Herd_immunity), also that part of the population that is not yet, do not want to be or cannot be vaccinated is protected. 

Small infants (before the time when they can be vaccinated), people with special medical conditions preventing them to be vaccinated or people with personal beliefs that prevent them from accepting vaccination are in this way protected. 

But for *herd immunity* to work, the vaccination rate should be over a specific threshold (for each disease, depending on natural virality and other factors). 

The current data contains information for California schools, from 2000 to 2014 about the number of pupils admissions with immunization  for  Measles/Mumps/Rubella (MMR), Polio and Diphtheria/Tetanus/Pertussis (DTP). 
Also, it shows the number of Permanent Medical Exemption (PME) and Personal Beliefs Exemption (PBE) for each school. 

Another two data files contains the information on Pertussis cases from 5 years, between 2010 and 2015 and also infants Pertussis cases between 2014 and 2015.
...
We will try to understand the geographical distribution of the immunization rates and medical permanent or personal beliefs exemptions. We will also highlight the difference between private schools (or educational institutions) and other schools. And we will try to see if the name of the schools with lower immunization rates can teach us something about the possible reasons of the high personal beliefs exemption percents here. 
After that, we will try to analyze the possible correlations between low immunization rates and high Pertussis cases incidence.
...
Let's glimpse the data. We look to all 4 files imported. Click on each of the tabs below to inspect all the data files.
...
Then we will use **pertusisData** with **geoData** to understand if there is a correlation between low immunization rates, high PME and PBE rates and Pertussis incidence. 
...
We calculate percents for the Permanent Medical Exemption (PME), Personal Beliefs Exemption (PBE) from the number of such cases at each school (public and private). 
As well, we calculate similar percents for number of vaccinations with DTP, Polio and MMR.

From the dataset `county.regions` we extract the counties from `California`. We then merge the counties data with the student data. The data can be used then for map visualization.
...
We perform similar processing for the private schools only, by adding to previous processing a filter for `PRIVATE` schools.
...
In the following, let's analyze the data for the Permanent Medical Exemption (PME), Personal Beliefs Exemptiuon (PBE) and immunization rate for DTP, Polio and MMR. 
For each of these, we will look to the values aggregated per county and per year. 
The percent is calculated as number of occurences divided with the total number of school children (per county and per year).
...
The Permanent Medical Exemption (PME) is an exception to compulsory school immunization laws, based upon a medical condition. We will look to the percent of PME for each county and year from total of school children registered.
...
We can notice that the average values by year are ranging from 0% to 8%, largest values being recorded in 2004 and 2006.
...
We can observe that the maximum average values aggregated by county are obtained in counties like Marin, Modo, Siskiyou, all with low population and rather secluded.
...
From this treemap representation we can see that in counties with large population, the average percent of permanent medical exemption (PME) is small and maximum average values are indeed in counties with very small population.




```{r plotly_permanant_medical_exemption_rates, warnings=FALSE}



mapPMEbyYear <- function(nYear,datafile) {

  

    datafile %>% filter(year==nYear) %>% select(county,year,pPME) %>% ungroup() -> county_pme
    ...
    cali_p <- merge(cali, county_pme, by.x = "subregion", by.y = "county", all.y=TRUE)

    

    

    minM = min(cali_p$pPME); maxM = max(cali_p$pPME); deltaM = (maxM - minM)/10

    

    cali_p$ppPME <- cut(cali_p$pPME, breaks = c(seq(minM, maxM, by = deltaM)))
    ...
    cali_p %>%
      ...
      plot_ly(x = ~long, y = ~lat, color = ~ppPME, colors = c('green','tomato'),
      ...
      layout(
        ...
        title = paste0("California Permanent Medical Exemption rates\n (averaged by County, year: ",nYear, ")"),
...
Click on the tabs below to inspect the map plots for each year.



#####2000

```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2000, warnings=FALSE}

mapPMEbyYear(2000,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2001, warnings=FALSE}

mapPMEbyYear(2001,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2002, warnings=FALSE}

mapPMEbyYear(2002,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2003, warnings=FALSE}

mapPMEbyYear(2003,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2004, warnings=FALSE}

mapPMEbyYear(2004,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2005, warnings=FALSE}

mapPMEbyYear(2005,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2006, warnings=FALSE}

mapPMEbyYear(2006,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2007, warnings=FALSE}

mapPMEbyYear(2007,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2008, warnings=FALSE}

mapPMEbyYear(2008,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2009, warnings=FALSE}

mapPMEbyYear(2009,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2010, warnings=FALSE}

mapPMEbyYear(2010,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2011, warnings=FALSE}

mapPMEbyYear(2011,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2012, warnings=FALSE}

mapPMEbyYear(2012,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2013, warnings=FALSE}

mapPMEbyYear(2013,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_permanent_medical_extempt_rates2014, warnings=FALSE}

mapPMEbyYear(2014,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2001, warnings=FALSE}

mapPBEbyYear(2001,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2003, warnings=FALSE}

mapPBEbyYear(2003,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2005, warnings=FALSE}

mapPBEbyYear(2005,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2007, warnings=FALSE}

mapPBEbyYear(2007,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2009, warnings=FALSE}

mapPBEbyYear(2009,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2011, warnings=FALSE}

mapPBEbyYear(2011,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_personal_beliefs_extempt_rates2013, warnings=FALSE}

mapPBEbyYear(2013,cYSD)
...
```{r plotly_dtp_immun_rates, warnings=FALSE}
...
mapDTPbyYear <- function(nYear,datafile) {
    ...
    cali_p %>%
      ...
      layout(
        ...
        title = paste0("California Diphtheria/Tetanus/Pertussis immunization rates\n (averaged by County, year: ",nYear, ")"),
...
```{r plotly_polio_immun_rates, warnings=FALSE}



mapPoliobyYear <- function(nYear,datafile) {

  

    datafile %>% filter(year==nYear) %>% select(county,year,pPolio) %>% ungroup() -> county_pol
    ...
    cali_p <- merge(cali, county_pol, by.x = "subregion", by.y = "county", all.y=TRUE)

    

    

    minM = min(cali_p$pPolio); maxM = max(cali_p$pPolio); deltaM = (maxM - minM)/10

    

    cali_p$ppPolio <- cut(cali_p$pPolio, breaks = c(seq(minM, maxM, by = deltaM)))
    ...
    cali_p %>%
      ...
      plot_ly(x = ~long, y = ~lat, color = ~ppPolio, colors = c('tomato','green'),
      ...
      layout(
        ...
        title = paste0("California Polio immunization rates\n (averaged by County, year: ",nYear, ")"),
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2000, warnings=FALSE}

mapPoliobyYear(2000,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2001, warnings=FALSE}

mapPoliobyYear(2001,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2002, warnings=FALSE}

mapPoliobyYear(2002,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2003, warnings=FALSE}

mapPoliobyYear(2003,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2004, warnings=FALSE}

mapPoliobyYear(2004,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2005, warnings=FALSE}

mapPoliobyYear(2005,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2006, warnings=FALSE}

mapPoliobyYear(2006,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2007, warnings=FALSE}

mapPoliobyYear(2007,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2008, warnings=FALSE}

mapPoliobyYear(2008,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2009, warnings=FALSE}

mapPoliobyYear(2009,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2010, warnings=FALSE}

mapPoliobyYear(2010,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2011, warnings=FALSE}

mapPoliobyYear(2011,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2012, warnings=FALSE}

mapPoliobyYear(2012,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2013, warnings=FALSE}

mapPoliobyYear(2013,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_polio_immun_rates2014, warnings=FALSE}

mapPoliobyYear(2014,cYSD)
...
```{r plotly_mmr_immun_rates, warnings=FALSE}



mapMMRbyYear <- function(nYear,datafile) {

  

    datafile %>% filter(year==nYear) %>% select(county,year,pMMR) %>% ungroup() -> county_mmr
    ...
    cali_p <- merge(cali, county_mmr, by.x = "subregion", by.y = "county", all.y=TRUE)

    

    

    minM = min(cali_p$pMMR); maxM = max(cali_p$pMMR); deltaM = (maxM - minM)/10

    

    cali_p$ppMMR <- cut(cali_p$pMMR, breaks = c(seq(minM, maxM, by = deltaM)))
    ...
    cali_p %>%
      ...
      plot_ly(x = ~long, y = ~lat, color = ~ppMMR, colors = c('tomato','green'),
      ...
      layout(
        ...
        title = paste0("California Measles/Mumps/Rubella immunization rates\n (averaged by County, year: ",nYear, ")"),
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2000, warnings=FALSE}

mapMMRbyYear(2000,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2001, warnings=FALSE}

mapMMRbyYear(2001,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2002, warnings=FALSE}

mapMMRbyYear(2002,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2003, warnings=FALSE}

mapMMRbyYear(2003,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2004, warnings=FALSE}

mapMMRbyYear(2004,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2005, warnings=FALSE}

mapMMRbyYear(2005,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2006, warnings=FALSE}

mapMMRbyYear(2006,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2007, warnings=FALSE}

mapMMRbyYear(2007,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2008, warnings=FALSE}

mapMMRbyYear(2008,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2009, warnings=FALSE}

mapMMRbyYear(2009,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2010, warnings=FALSE}

mapMMRbyYear(2010,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2011, warnings=FALSE}

mapMMRbyYear(2011,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2012, warnings=FALSE}

mapMMRbyYear(2012,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2013, warnings=FALSE}

mapMMRbyYear(2013,cYSD)
...
```{r fig.width=6, fig.height=4, plotly_mmr_immun_rates2014, warnings=FALSE}

mapMMRbyYear(2014,cYSD)
...
It appears to be several clusters of schools with extremely low DTP immunization rates (in different years). One cluster is formed around San Diego another one around Los Angeles and another two around San Francisco and Sacramento.
...
We represent now the maps with DTP immunization rates (average values) per county, for the years from 2000 to 2014.
Click on the tabs below to inspect the map plots for each year.  
...
Let's try to represent the Voronoi polygons around the schools. This will give us a good image of the relative influence zones of each school. We will pick one year, **2012**, when there was the minimum average of immunization rate for DTP. This is one year for which we also have the Pertussis cases. We will show on the map the Voronoi polygons around the schools and also the details about schools.
...
# ToDo: find a way to filter external polygons so that we will only show the inner polygons
...
It appears that in some of the counties with persistent low immunization rate high percents of Pertussis cases can appear (Nevada, Modoc, Humboldt, Mariposa, Tuolumne); there are notably exceptions: Mono appears twice in top 3 for most higher percent of Pertussis cases per county population and is not present in the top 30 of lowest DTP immunization. Siskiyou appers several times in top 30 of county (years) with low immunization rates and is not present in top 30 of high Pertussis cases per county population.

With the Voronoi polygons representation of the each school influence area we can see how compact zones of low immunization rates could trigger Pertussis outbreaks. We only represented these areas for the year **2012**.


#**Final conclusions**


Public and private schools exhibit low averages per county and year for **Permanent Medical Exemption** (**PME**). 
The number of **Personal Beliefs Exemption** (**PBE**) increased very much in recent years. Private schools shows generally larger numbers for **PME** and **PBE**. In counties with large population concentration, like Los Angeles, San Diego or Orange these values exceeds even 10%. Immunization rates (average values) ranges between 70% and 95%, with extremes values as low as 50% in some average per county and per year. If we are not looking to the averages per county and per year but to  extreme values per school, we noticed that there are numerous schools with very high **PME** and **PBE** and with very low immunization rates (even 0%). Looking to what is special in those schools, we discovered that most of the schools with very low immunization rates are either **confessional education institutions**, **charter schools** or **special education institutions**  (**Montessori**, **Waldorf**). One pattern we observed was that typically the lowest  immunisation rates appears in schools from low populated, isolated and poor areas (Nevada, Modoc, Humboldt, Mariposa, Tuolumne) while there are numerous high urbanized areas (with lots of special education or confesional education institutions) with low immunisation rates.
...
Please give me your feedback for improvement of this Kernel. If you <font color="green"><b>like it</b></font>, please also press the <font color="red"><b>magic</b></font> button.
...
[9] California Immunization Registry, http://cairweb.org/  
[10] California Department of Public Health, Immunization Branch, https://www.cdph.ca.gov/Programs/CID/DCDC/Pages/immunize.aspx  
